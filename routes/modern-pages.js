...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* استایل‌های جدید برای اخبار و بازار */
        .news-grid {
            display: grid;
        ...
        <div class="content-grid">
            ...
            <div class="content-card">
                ...
                <div style="margin-top: 15px">
                    ...
                    <div>• 50+ کندل‌استیک</div>
                    <div>• 100% سلامت سیستم</div>
                </div>
            </div>
        </div>
        ...
        <script>
            async function runScan() {
                ...
                try {
                    ...
                    if (data.success && data.data) {
                        ...
                        document.getElementById('scanResult').innerHTML = \`
                            ...
                            <div style="margin-top: 10px">
                                <div>تعداد ارزها: \${scanData.length || scanData.count || '0'}</div>
                                <div>اولین ارز: \${scanData[0]?.name || scanData[0]?.symbol || 'N/A'}</div>
                                <div>قیمت: \${scanData[0]?.price ? '$' + formatPrice(scanData[0].price) : 'N/A'}</div>
                                <div>تغییر 24h: \${scanData[0]?.priceChange24h ? scanData[0].priceChange24h + '%' : 'N/A'}</div>
                            </div>
                        \`;
                    } else {
                    ...
                    document.getElementById('scanResult').innerHTML = '<div class="status-indicator error">❌ خطا در اسکن: ' + error.message + '</div>';
                }
            }
        </script>`;
        ...
        <script>
            async function loadAnalysis() {
                ...
                try {
                    ...
                    document.getElementById('analysisResult').innerHTML = '<div class="status-indicator error">❌ خطا در ارتباط: ' + error.message + '</div>';
                }
            }
            ...
            function displayAnalysis(data) {
                const html = \`
                    <div style="margin-top: 15px;">
                        ...
                        <div class="metric-grid">
                            <div class="metric-card">
                                <div class="metric-value">\${data.current_price ? '$' + formatPrice(data.current_price) : 'N/A'}</div>
                                <div class="metric-label">قیمت (USD)</div>
                            </div>
                            <div class="metric-card">
                                <div class="metric-value">\${data.indicators?.rsi || data.rsi || '0'}</div>
                                <div class="metric-label">RSI</div>
                            </div>
                            <div class="metric-card">
                                <div class="metric-value">\${data.signals?.trend || data.trend || 'NEUTRAL'}</div>
                                <div class="metric-label">روند</div>
                            </div>
                            <div class="metric-card">
                                <div class="metric-value">\${data.signals?.strength || data.strength || '0'}%</div>
        ...
        <script>
            async function loadMarketData() {
                try {
                    ...
                    const response = await fetch('/api/markets/live?limit=15');
            ...
            async function loadTopGainers() {
                try {
                    ...
                    if (data.success && data.data && data.data.length > 0) {
                        ...
                        throw new Error('داده‌ای برای برترین سودها دریافت نشد');
                    }
                } catch (error) {
                    document.getElementById('marketResult').innerHTML = 
                ...
                try {
                    ...
                    if (data.success && data.data) {
                        ...
                        document.getElementById('marketCapResult').innerHTML = \`
                            ...
                            <div style="margin-top: 10px;">
                                <div>💰 مارکت کپ: $\${formatNumber(globalData.total_market_cap || globalData.market_cap)}</div>
                                <div>📊 حجم معاملات: $\${formatNumber(globalData.total_volume || globalData.volume_24h)}</div>
                                <div>🏪 تسلط بیت‌کوین: \${globalData.btc_dominance || globalData.bitcoin_dominance || 'N/A'}%</div>
                                <div>📈 تعداد ارزها: \${globalData.active_cryptocurrencies || globalData.active_crypto || 'N/A'}</div>
                            </div>
                        \`;
                    } else {
                        throw new Error('داده‌ای برای مارکت کپ دریافت نشد');
                    }
                } catch (error) {
                    document.getElementById('marketCapResult').innerHTML = 
        ...
        <script>
            async function loadDashboard() {
                try {
                    ...
                    if (data.success && data.data) {
                        ...
                        const btcDominance = dashboard.btc_dominance?.value || dashboard.btc_dominance || dashboard.btcDominance || 'N/A';
                        const fearGreed = dashboard.fear_greed?.value || dashboard.fear_greed || dashboard.fearGreedIndex || 'N/A';
                        const marketCap = dashboard.market_cap?.total || dashboard.market_cap || dashboard.total_market_cap || 'N/A';
            ...
            async function loadFearGreed() {
                try {
                    ...
                    if (data.success && data.data) {
                        const value = data.data?.value || data.data?.fear_greed_value || data.data || 'N/A';
            ...
            async function loadBTCDominance() {
                try {
                    ...
                    if (data.success && data.data) {
                        const dominance = data.data?.value || data.data?.dominance || data.data || 'N/A';
                ...
                try {
                    ...
                    if (data.success && data.data && data.data.length > 0) {
                        ...
                        document.getElementById('newsResult').innerHTML = 
                            '<div class="status-indicator">📭 خبری یافت نشد</div>';
                    }
                } catch (error) {
                    document.getElementById('newsResult').innerHTML = 
            ...
            async function loadTrendingNews() {
                try {
                    ...
                    if (data.success && data.data && data.data.length > 0) {
                        ...
                        document.getElementById('newsResult').innerHTML = 
                            '<div class="status-indicator">📭 خبر ترندی یافت نشد</div>';
                    }
                } catch (error) {
                    document.getElementById('newsResult').innerHTML = 
                    ...
                    if (data.success && data.data) {
                        ...
                        document.getElementById('healthResult').innerHTML = \`
                            ...
                            <div style="margin-top: 15px;">
                                ...
                                <div>🟢 WebSocket: \${health.websocket_status?.status || 'نامشخص'}</div>
                                <div>🟢 دیتابیس: \${health.gist_status?.status || 'نامشخص'}</div>
                                <div>🟡 API: \${health.api_status?.status || 'نامشخص'}</div>
                                <div>🟢 سیستم: \${health.system_status?.status || 'نامشخص'}</div>
                            </div>
                        \`;
                    } else {
                    ...
                    if (data.success && data.data) {
                        document.getElementById('healthTabResult').innerHTML = \`
                            ...
                            <div style="margin-top: 10px;">
                                ...
                                <div>🟢 WebSocket: \${data.data.websocket_status?.status || 'نامشخص'}</div>
                                <div>🟢 API: \${data.data.api_status?.status || 'نامشخص'}</div>
                                <div>🟢 دیتابیس: \${data.data.gist_status?.status || 'نامشخص'}</div>
                            </div>
                        \`;
                    } else {
                        throw new Error(data.error || 'خطا در بررسی سلامت');
